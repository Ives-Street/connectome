...
def prepare_mode_settings(**kwargs):
    ...
    # Note: r5py accepts string mode names (e.g. "WALK", "CAR", "TRANSIT", "BICYCLE")
    # for transport_modes/access_modes.
...
def route_scenario(folder_name, 
    ...
    # detailed_itineraries = DetailedItineraries(
    #     scenario_transport_network, 
    #     pop_points,
    #     pop_points,
    #     **mode_settings['TRANSIT'],
    #     )
    # travel_time_matrix_detailed = pd.DataFrame(detailed_itineraries)
    ...
    for mode in modes:
        ...
        ttm_long = pd.DataFrame(ttm)
        ttm_wide = pd.pivot(ttm_long, index='from_id', columns='to_id', values='travel_time')
        ttm_wide.to_csv(f'{folder_name}/results/{mode}_ttm.csv')
